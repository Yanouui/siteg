...
$select="select * from insc where  email='$email' ";
$query=  mysqli_query($con, $select);
//$x=mail($to, $subject, $message);
if(mysqli_num_rows($query)>0){   
$row=  mysqli_fetch_array($query);
if($row["password"]==$password) {
echo '<script>alert("bienvenu");window.location.assign("home.php");</script>';
 } 
else{
 echo '<script>alert("Le mot de pass incorrect");</script>';

   }
    }
 else{
  echo '<script>alert("lemail nexiste pas merci de vous inscrire");window.location.assign("signup.php");</script>';
  ...
  color: rgb(3, 9, 119) ;
    ...
    <header>
       ...
       <div class="forminscriptionN">
        ...
        <input type="submit" name="signin" value="Connexion" class="btn_inscr">
